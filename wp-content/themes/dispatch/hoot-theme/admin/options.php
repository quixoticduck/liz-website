...
function hootoptions_intro_buttons( $buttons ) {
	$buttons = array(
		...
		'rate' =>    array( 'text'   => __( '5<i class="fa fa-star"></i> Rate the Theme', 'hoot'),
							'button' => 'secondary',
							'url'    => 'https://wordpress.org/themes/dispatch/',
							'icon'   => 'none' ),
		);
...
function hootoptions_options() {
	...
	$options[] = array(
		...
		'desc' => sprintf( __('Use %sSite Title%s as text logo', 'hoot'), '<a href="' . admin_url('/') . 'options-general.php" target="_blank">', '</a>' ),
		'id' => 'logo',
		'class' => 'logo_selector',
		'std' => 'text',
		'type' => 'radio',
		'options' => array(
			'text' => __('Plain Text', 'hoot'),
	...
	$options[] = array(
		'name' =>  __('Site Box Background', 'hoot'),
		'desc' => __("This background will be used only if one of the <strong>'boxed'</strong> options is selected in the <strong>'Site Layout'</strong> option in the Layout tab above.", 'hoot'),
		'id' => 'box_background',
		'std' => array(
			'color' => '#ffffff', ),
		'type' => 'background', );
	...
	$options[] = array(
		'name' => __('Left Topbar', 'hoot'),
		'desc' => sprintf( __('You can add content to Left Topbar using Text Widget in the %sWidget Management Screen%s', 'hoot'), '<a href="' . admin_url('/') . 'widgets.php" target="_blank">', '</a>' ),
		'type' => 'info');
	...
	$options[] = array(
		...
		'type'		=> 'group',
		'settings' => array(
			'title' => __('Profile', 'hoot'),
			'add_button' => __('Add Another Profile', 'hoot'),
			'remove_button' => __('Remove Profile', 'hoot'),
			'repeatable' => true,
			'sortable' => true,
			'toggleview' => true, ),
		'fields'	=> array(
	...
	$options[] = array(
		'name' => __('Meta Information for Post Page', 'hoot'),
		'desc' => __('Check which meta information to display on single post page.', 'hoot'),
		'id' => 'post_meta',
		'std' => array(
			'author' => '1',
			'date' => '1',
	...
	$options[] = array(
		...
		'fields' => array(
			array(
				'name' => __('To hide this slide, simply leave the Image, Title and Content empty.', 'hoot'),
				'type' => 'info'),
			array(
			...
			array(
				'name' => __('Link Text', 'hoot'),
				'id' => 'button',
				'type' => 'text'),
			array(
				'name' => __('Link URL', 'hoot'),
				'desc' => __('Leave empty if you do not want to show the link.', 'hoot'),
				'id' => 'url',
				'type' => 'text'),
			array(
				'name' =>  __('Slide Background', 'hoot'),
				'desc' => __('This can be useful if you are using transparent images', 'hoot'),
				'id' => 'background',
				'std' => '#ffffff',
				'type' => 'color', ),
			), );
